<x-app-layout>
    <title>SmartSIM - Bulk Assign SIM</title>
    ...
    <div class="max-w-3xl mx-auto space-y-6"
         x-data="{
            ...
            async validateNumbers() {
                ...
                const res = await fetch('{{ route('admin.sim-plan.bulk-assign.resolve-numbers') }}', {
                    method: 'POST',
                    headers: {
        ...
        <div class="flex items-center gap-3 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <div class="w-10 h-10 rounded-2xl bg-emerald-50 border border-emerald-100/50 flex items-center justify-center text-emerald-600 shrink-0">
                <i data-lucide="user-plus" class="w-5 h-5"></i>
            </div>
            <div>
                <h1 class="text-xl font-extrabold font-display text-slate-900">Bulk Assign SIM</h1>
                <p class="text-sm text-slate-500 mt-0.5">Paste a batch of SIM numbers, review what's valid, then pick who to hand them to.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4" x-show="validated && resolved.length > 0" x-cloak>
            <h3 class="text-sm font-semibold text-slate-800 font-display pb-3 border-b border-slate-100">2. Assign To</h3>
            ...
            <button type="button" @click="submit()" :disabled="!selectedUser"
                    class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-colors">
                <i data-lucide="user-plus" class="w-3.5 h-3.5"></i>
                Assign <span x-text="resolved.length"></span> SIM(s) to Selected User
            </button>
        </div>
    </div>
</x-app-layout>
